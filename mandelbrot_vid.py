...
class LoadImage:
    ...
    def superdraw(self,event):
        ...
        for i in range(len(self.PATH)-1):
            fpz=40
            for zoom in range(self.PATH[i][2]*fpz,self.PATH[i+1][2]*fpz+1):
                z=zoom/fpz
                r=((z-self.PATH[i][2])/(self.PATH[i+1][2]-self.PATH[i][2]))**.5
                x=shyft(self.PATH[i][0],self.PATH[i+1][0],r)
                y=shyft(self.PATH[i][1],self.PATH[i+1][1],r)
                '''
                print("z",z,self.PATH[i+1][2],z-self.PATH[i][2],self.PATH[i+1][2]-self.PATH[i][2],r
                      ,"\nx",x,self.PATH[i][0],self.PATH[i+1][0]
                      ,"\ny",y,self.PATH[i][1],self.PATH[i+1][1])
                '''
                Graph=draw_graph(y,x,z,size,self.JobsQ,self.ReturnQ)
                        
                img = Image.new('RGB', (size, size))
                img.putdata(Graph)
                img.save('SuperMandelbrot'+str(zoom).zfill(7)+'.png')
                
                print(zoom,"out of",self.PATH[-1][2]*fpz)
    ...
    def zoomin(self,event):
        ...
        if self.zoomcycle==None:
            ...
            if x<=self.SIZE and y<=self.SIZE:
                ...
                self.PATH+=[(self.center_x,self.center_y,self.zoom)]
...
def mandel_math(x,y,zoom):
    pre_i=0
    pre_r=0
    for loops in range(1, int(5*zoom**2+zoom*30+50) ):
